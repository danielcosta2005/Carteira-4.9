...
export default function QRTab({ project, setProject }) {
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [claimUrl, setClaimUrl] = useState('');
  const [claimUrlTemplate, setClaimUrlTemplate] = useState(project.claim_url_template || '');
  const [qrPayloadTemplate, setQrPayloadTemplate] = useState(project.qr_payload_template || '');
      ...
      <div className="grid md:grid-cols-2 gap-8 items-center">
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-2">Compartilhe este link com seus clientes para que eles possam resgatar o passe de fidelidade.</p>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-4 pt-4 border-t">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">Use {'{projectId}'} e {'{googleSub}'}. Deixe em branco para usar o padrão.</p>
            </div>
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">Template para leitura em scanners. Use {'{projectId}'} e {'{googleSub}'}.</p>
            </div>
